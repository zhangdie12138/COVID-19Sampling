...
## dates of simulation
input_dates = seq(startdate, enddate, by="days") # Guangzhou/Beijing

## communities
begin.pat <- fread('sim_begin_1pts.csv', stringsAsFactors = F)
...
for (ii in 1:30){
  ...
  ## selected one location based on the population density
  begin.p <- begin.pat[sample(seq_len(nrow(begin.pat)),1,prob=begin.pat$popdensity),]
  patnInf[which(patNames%in%begin.p$jd_id)] = 1
  ...
  save(results, file=paste0(as.character(ii),"_results_50t.RData"))
  print(ii)
